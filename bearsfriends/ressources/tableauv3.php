...
echo '<caption>Tableau avec grand nombre de donné ';
   echo'<thead>';
       ...
       echo'</tr>';
  echo'</thead>';
//require 'mailgraph.php';      //variable d'incrementation de mes ref
...
$lignes[] = array (string2url(($titre[1][$j]),chr(0xEF).chr(0xBB).chr(0xBF)),$ref['ref'.($i+1)],$vendeur[1][$j],floatval(str_replace(",",".",rtrim ($prix[1][$j]))));
...
for($j = 0; $j < count($prix[1]); $j++) // On parcourt le tableau $prix[1]
{
   echo'<tbody>';
       echo'<tr>';
          echo '<td> '.html_entity_decode($titre[1][0]).' </td>';
          echo '<td> '.$ref['ref'.($i+1)].' </td>';
          echo '<td> '.$vendeur[1][$j].' </td>';
          echo '<td> '.$prix[1][$j].' </td>';
